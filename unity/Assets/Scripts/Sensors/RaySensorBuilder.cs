...
public static class RaySensorBuilder
{
    ...
    public static void Draw360RayGizmos(
        BaseAgent observer,
        int rayCount,
        float maxDistance,
        LayerMask raycastMask,
    ...
    {
        int safeRayCount = Mathf.Max(0, rayCount);
        if (observer == null || safeRayCount == 0)
        {
            return;
        }
        ...
        for (int i = 0; i < safeRayCount; i++)
        {
            float angle = safeRayCount <= 1 ? 0f : (360f / safeRayCount) * i;
            Vector3 direction = Quaternion.Euler(0f, angle, 0f) * observer.transform.forward;
            RaySensorReading reading = CastRay(observer, direction.normalized, maxDistance, raycastMask, originHeight);
            float distance = reading.NormalizedDistance * Mathf.Max(0.001f, maxDistance);
            DebugDrawUtils.DrawRay(origin, reading.Direction, distance, GetDebugColor(reading.HitType));
        }
    }
    ...
    private static Color GetDebugColor(RaySensorHitType hitType)
    {
        switch (hitType)
        {
            case RaySensorHitType.Wall:
                return Color.gray;
            case RaySensorHitType.Exit:
                return Color.green;
            case RaySensorHitType.Teammate:
                return Color.cyan;
            case RaySensorHitType.Opponent:
                return Color.red;
            case RaySensorHitType.Other:
                return Color.yellow;
            default:
