...
public struct EnvRuleConfig
{
    ...
    public bool UseUnseenMazeLayout;
...
public struct ObservationConfig
{
    ...
    public bool IncludeOpponentSummary;
    ...
    public static ObservationConfig Default()
    {
        return new ObservationConfig
        {
            ...
            IncludeExitVector = true,
            IncludeOpponentSummary = true,
            SentinelRayCount = 14,
            RunnerRayCount = 16
        };
    }
...
public static class EnvRuleConfigLoader
{
    public static bool TryLoad(string configPath, out EnvRuleConfig config, out string message)
    {
        ...
        config.UseUnseenMazeLayout = GetBool(values, "maze.use_unseen_layout", config.UseUnseenMazeLayout) ||
            string.Equals(GetString(values, "maze.layout_split", string.Empty), "unseen", StringComparison.OrdinalIgnoreCase);
        ...
        observationConfig.IncludeOpponentSummary = GetBool(
            values,
            "observations.include_opponent_summary",
            observationConfig.IncludeOpponentSummary);
