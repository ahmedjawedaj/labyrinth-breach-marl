...
The study is organized around three research questions. \textbf{RQ1} asks whether a staged curriculum can produce competent policies as navigation complexity increases from an open arena to a dynamically changing maze. \textbf{RQ2} asks how strongly a fixed policy depends on maze geometry when transferred to held-out procedural layouts. \textbf{RQ3} asks which design elements, including memory, tactical reward shaping, wall dynamics, curriculum transfer, and action assistance, are responsible for observed performance and coordination. The current artifact answers the fixed-count five-seed evaluation component of RQ2 and the deployment-time portions of RQ3 for dynamic walls and action assistance. The curriculum, memory, and tactical-reward mechanism claims remain open because their paired ablations require retraining.

This distinction between a \emph{benchmark contribution}, a \emph{validated diagnostic}, and a \emph{completed empirical study} is important. The environment, training pipeline, canonical evaluation, and two intervention analyses are implemented and auditable. Three retraining ablations remain incomplete, so the paper frames Labyrinth Breach as a reproducible simulation benchmark and empirical progress report rather than a finished claim that the learned controller is state of the art.
...
evaluation cells. After the no-retraining polish pass, the publication-readiness
audit passes 15/16 gates because the canonical evaluation audit is valid and
the manuscript PDF is routed correctly. The remaining blocker is paired
ablation completeness: only two of five planned paired analyses are complete.
Consequently, this section reports the completed evidence honestly and does not
claim completion of the full journal experiment matrix.
...
using one evaluation seed and 25 target episodes per split. These rows are
diagnostic, not a replacement for the five-seed canonical evaluation. They show
that the learned PPO policy is substantially stronger than random pursuit on
held-out capture time and coordination prevalence, but a hand-coded geometric
controller still captures faster and wins more often. The defensible conclusion
is therefore modest: the benchmark is not solved by the learned policy, and a
future journal submission should include at least one matched-compute learned
MARL baseline in addition to these rule-based controls.

\begin{table*}[t]
\centering
\caption{Low-cost evaluation-only baselines. Learned PPO uses the canonical
five-policy-seed assist-off evaluation from Table~\ref{tab:official_eval_pause}.
...
It is not a controlled generalization experiment. First, all cells reuse one policy pair. Second, a fixed 30-second wall-clock budget produces unequal sample counts from 12 to 31 episodes per configuration, so an episode-weighted aggregate gives greater influence to configurations with faster termination. Third, all unseen configurations reuse one topology, and deterministic inference makes nominal evaluation-seed repetitions partially redundant. Fourth, the legacy split comparison changes movement and wall parameters together with the scene. The official evaluation therefore uses a fixed completed-episode target per policy-topology cell, independent policy seeds, genuinely distinct seeded topologies, identical non-topology controls, and explicit spawn/episode randomization. Until those runs finish, Table~\ref{tab:per_layout} is a diagnostic table rather than the primary result of the paper.
...
\textbf{Submission scope.} The strongest current venue framing is a reproducible
simulation benchmark and empirical study for multidisciplinary robotics and AI
readers. The manuscript should not be submitted as a hardware robotics paper,
nor as a new MARL algorithm paper. The defensible novelty is the combination of
asymmetric learned teams, dynamic topology, exit-directed evasion, partial
observation, explicit reward provenance, and artifact-gated evaluation. The
remaining retraining ablations determine whether the paper can claim mechanism
evidence for memory, tactical rewards, and curriculum transfer.
...
The current PDF is generated from the journal \LaTeX{} source under \texttt{paper/}. Official evaluation results are read from the publication-evaluation aggregate directory. Learning curves and paired ablations are read from the official-summary directory. The codebase includes unit tests under \texttt{tests/}. The no-retraining evidence snapshot is generated by the current-evidence snapshot script. The evidence-pack builder creates a hashed archive with \texttt{artifact\_manifest.json} and \texttt{SHA256SUMS}, but it is intentionally blocked for a formal submission pack until all readiness gates pass. This prevents an incomplete ablation matrix from being packaged as final journal evidence.

\begin{table*}[t]
\centering
\caption{Claim-to-artifact traceability for the current no-retraining manuscript.}
...
Reward confounding & Reward events are logged and paired tactical-reward retraining remains a required gate \\
...
Future work should prioritize the three remaining retraining ablations, a
matched-compute learned MARL baseline, recurrent or belief-state memory, and
stress tests for wall-transition safety. SHAP-style attribution is also useful,
but it should group features by semantic channels before producing visual
explanations.
...
We presented Labyrinth Breach, a reproducible Unity ML-Agents benchmark for asymmetric multi-agent pursuit-evasion with dynamic topology, partial observation, exit-directed evasion, and tactical reward shaping. The upgraded evidence now includes audited five-seed training curves, a 30-cell canonical assist-off evaluation, lightweight diagnostic baselines, and two completed deployment-time interventions. The benchmark is not solved: Runner wins and escapes remain common, and a geometric heuristic captures faster than the current PPO policy. This makes Labyrinth Breach useful as a controlled benchmark, while the remaining retraining ablations define the path from advisor-review draft to final journal evidence package.
